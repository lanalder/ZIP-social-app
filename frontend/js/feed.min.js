...
$(document).ready(function(){let t,s=void new Array(6);$.ajax({url:"config.json",type:"GET",dataType:"json",success(e){t=`${e.SERVER_URL}:${e.SERVER_PORT}`,a()},error(e){console.log(e)}});const i=e=>{console.log(e),$.ajax({url:`${t}/seeComments/${s}`,type:"GET",dataType:"json",context:e.parentElement.parentElement.parentElement,success(t){for(let e=0;e<t.length;e++){var a=t[e];this.innerHTML+=`
        ...
        <div class="form-group mb-3">
          ...
          <p class="text-end send-comment-btn"><i class="fa fa-paper-plane text-end" aria-hidden="true"></i></p>
        </div>`},error(e){console.log(e)}})},o=e=>{sessionStorage.getItem("user_id")?$.ajax({url:`${t}/likePost/${s}`,type:"POST",data:{user_id:sessionStorage.getItem("user_id")},dataType:"json"}).done(e=>{console.log(e)}):alert("Please login or register to interact with posts :)")},a=()=>{$.ajax({url:`${t}/allPosts`,type:"GET",dataType:"json",context:document.querySelector("#albumOutput"),success(t){this.innerHTML="";for(let e=t.length-1;0<=e;--e){var a=t[e];this.innerHTML+=`<div class="col-md-4">
              <div class="card mb-4">
                <img class="card-img-top" src="${a.img_url}" alt="" style="width: 100%">
                <div class="card-img-overlay d-flex">
                  <div class="expand-container text-wrap">
                    <i class="${a._id} fa fa-expand" aria-hidden="true" data-bs-toggle="modal" data-bs-target="#viewFullImageModal"></i>
                  </div>
            ...
            </div>`}Array.from(document.querySelectorAll(".interaction-icon")).forEach(t=>{t.addEventListener("click",function(e){s=e.target.classList[0],e.target.classList.contains("comment")?i(t):e.target.classList.contains("like")&&o(t)},!1)})}})}});