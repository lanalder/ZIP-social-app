...
$(document).ready(function(){let a,i=void new Array(6);$.ajax({url:"config.json",type:"GET",dataType:"json",success(e){a=`${e.SERVER_URL}:${e.SERVER_PORT}`,t()},error(e){console.log(e)}});const s=e=>{console.log(e),$.ajax({url:`${a}/seeComments/${i}`,type:"GET",dataType:"json",context:e.parentElement.parentElement.parentElement,success(t){for(let e=0;e<t.length;e++){var a=t[e];this.innerHTML+=`
        ...
        <div class="form-group mb-3">
          ...
          <p class="text-end send-comment-btn"><i class="fa fa-paper-plane text-end" aria-hidden="true"></i></p>
        </div>`},error(e){console.log(e)}})},n=t=>{sessionStorage.getItem("user_id")?$.ajax({url:`${a}/likePost/${i}`,type:"POST",data:{user_id:sessionStorage.getItem("user_id")},dataType:"json"}).done(e=>{console.dir(t),t.firstChild.classList.toggle("fa-heart-o"),t.firstChild.classList.toggle("fa-heart"),t.firstChild.classList.toggle("active-icon")}):alert("Please login or register to interact with posts :)")};const t=()=>{$.ajax({url:`${a}/allPosts`,type:"GET",dataType:"json",context:document.querySelector("#albumOutput"),success(t){this.innerHTML="";for(let e=t.length-1;0<=e;--e){var a=t[e];this.innerHTML+=`<div class="col-md-4">
              <div class="card mb-4">
                <img class="card-img-top" src="${a.img_url}" alt="" style="width: 100%">
                <div class="card-img-overlay d-flex">
                  <div class="expand-container text-wrap">
                    <i class="${a._id} interaction-icon view fa fa-expand" aria-hidden="true" data-bs-toggle="modal" data-bs-target="#viewFullImageModal"></i>
                  </div>
            ...
            </div>`}Array.from(document.querySelectorAll(".interaction-icon")).forEach(t=>{t.addEventListener("click",function(e){i=e.target.classList[0],e.target.classList.contains("comment")?s(t):e.target.classList.contains("like")?n(t):e.target.classList.contains("view")?l(t):e.target.classList.contains("delete")&&console.log(e.target)},!1)})}})},l=e=>{document.querySelector(".fullImage").src=e.parentElement.parentElement.parentElement.firstElementChild.src,document.querySelector(".view-title").innerHTML=e.parentElement.nextElementSibling.lastElementChild.lastElementChild.innerText}});